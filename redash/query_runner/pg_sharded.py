...
class PostgreSQLSharded(BaseSQLQueryRunner):
    noop_query = "SELECT 1"
    ...
    def configuration_schema(cls):
        return {
            ...
            "properties": {
                ...
                #"defaultShard": {"type": "number", "default": 1},
                "sslmode": {
                    "type": "string",
                    "title": "SSL Mode",
                    "default": "prefer",
    ...
    def _get_connection(self):
        ...
        shardedHostName = self.configuration.get("host").format(Shard=self._get_shard())
        shardedDbName = self.configuration.get("dbname").format(Shard=self._get_shard())

        connection = psycopg2.connect(
            user=self.configuration.get("user"),
            password=self.configuration.get("password"),
            host=shardedHostName,
...
logger.debug("Registering query runners from pg_sharded.py.")

register(PostgreSQLSharded)